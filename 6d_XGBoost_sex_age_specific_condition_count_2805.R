...
print(table(xgb_data_count$gender, useNA = "ifany"))
...
run_xgb_subgroup <- function(data, subgroup_name, remove_gender = FALSE) {
  ...
  # Class weighting within subgroup
...
female_data <- xgb_data_count %>%
  filter(gender == 2 | gender == "female" | gender == "Female")

male_data <- xgb_data_count %>%
  filter(gender == 1 | gender == "male" | gender == "Male")

age_50_64_data <- xgb_data_count %>%
  filter(age2013 >= 50, age2013 <= 64)

age_65_74_data <- xgb_data_count %>%
  filter(age2013 >= 65, age2013 <= 74)

age_75plus_data <- xgb_data_count %>%
  filter(age2013 >= 75)

# Run subgroup models -----------------------------------------------------

xgb_count_female <- run_xgb_subgroup(
  data = female_data,
  subgroup_name = "XGBoost Count - Female",
  remove_gender = TRUE
)

xgb_count_male <- run_xgb_subgroup(
  data = male_data,
  subgroup_name = "XGBoost Count - Male",
  remove_gender = TRUE
)

xgb_count_age_50_64 <- run_xgb_subgroup(
  data = age_50_64_data,
  subgroup_name = "XGBoost Count - Age 50-64",
  remove_gender = FALSE
)

xgb_count_age_65_74 <- run_xgb_subgroup(
  data = age_65_74_data,
  subgroup_name = "XGBoost Count - Age 65-74",
  remove_gender = FALSE
)

xgb_count_age_75plus <- run_xgb_subgroup(
  data = age_75plus_data,
  subgroup_name = "XGBoost Count - Age 75+",
  remove_gender = FALSE
)

# Combine performance results --------------------------------------------

xgb_count_subgroup_performance <- bind_rows(
  xgb_count_female$performance,
  xgb_count_male$performance,
  xgb_count_age_50_64$performance,
  xgb_count_age_65_74$performance,
  xgb_count_age_75plus$performance
)

print(xgb_count_subgroup_performance)

# Optional: keep only Youden threshold rows for main subgroup table --------

xgb_count_subgroup_performance_youden <- xgb_count_subgroup_performance %>%
  filter(threshold_type == "Youden")

print(xgb_count_subgroup_performance_youden)

# Optional: keep only default threshold rows ------------------------------

xgb_count_subgroup_performance_050 <- xgb_count_subgroup_performance %>%
  filter(threshold_type == "Default 0.50")

print(xgb_count_subgroup_performance_050)

# Save results ------------------------------------------------------------

saveRDS(
  xgb_count_subgroup_performance,
  "xgb_count_subgroup_performance_all_thresholds.rds"
)

saveRDS(
  xgb_count_subgroup_performance_youden,
  "xgb_count_subgroup_performance_youden.rds"
)

saveRDS(
  xgb_count_subgroup_performance_050,
  "xgb_count_subgroup_performance_default_050.rds"
)

write.csv(
  xgb_count_subgroup_performance,
  "xgb_count_subgroup_performance_all_thresholds.csv",
  row.names = FALSE
)

write.csv(
  xgb_count_subgroup_performance_youden,
  "xgb_count_subgroup_performance_youden.csv",
  row.names = FALSE
)

write.csv(
  xgb_count_subgroup_performance_050,
  "xgb_count_subgroup_performance_default_050.csv",
  row.names = FALSE
)