...
# ============================================================
# 05c_xgboost_lca_tuned.R
# XGBoost tuned model: LCA modal class assignment
...
# Define common predictor variables ---------------------------------------
...
print(table(xgb_data_lca$eurodcat_w6, useNA = "ifany"))
print(table(xgb_data_lca$lca_label, useNA = "ifany"))
...
model_performance_xgb_lca_050 <- extract_performance(
  cm = cm_050,
  model_name = "XGBoost - LCA modal class",
  threshold_label = "Default 0.50",
  threshold_value = 0.5,
  auc_value = auc_value
)

model_performance_xgb_lca_youden <- extract_performance(
  cm = cm_youden,
  model_name = "XGBoost - LCA modal class",
...
model_performance_xgb_lca_comparison <- bind_rows(
  model_performance_xgb_lca_050,
  model_performance_xgb_lca_youden
)

print(model_performance_xgb_lca_comparison)

# Variable importance -----------------------------------------------------

varimp_xgb_lca <- xgb.importance(
  feature_names = colnames(x_train),
  model = xgb_lca_tuned
) %>%
  as_tibble()

print(varimp_xgb_lca)
...
p_shap <- ggplot(
  ...
  labs(
    title = "SHAP Feature Importance for the XGBoost LCA Model",
    x = NULL,
...
ggsave(
  filename = "shap_importance_xgb_lca.png",
  plot = p_shap,
  width = 10,
  height = 7,
  dpi = 300
...
saveRDS(xgb_lca_tuned, "xgb_lca_tuned_model.rds")
saveRDS(tuning_results, "tuning_results_xgb_lca.rds")
saveRDS(best_params, "best_params_xgb_lca.rds")

saveRDS(youden_coords_train, "youden_coords_train_xgb_lca.rds")
saveRDS(youden_threshold, "youden_threshold_xgb_lca.rds")

saveRDS(model_performance_xgb_lca_youden, "model_performance_xgb_lca_youden.rds")
saveRDS(model_performance_xgb_lca_050, "model_performance_xgb_lca_050.rds")
saveRDS(model_performance_xgb_lca_comparison, "model_performance_xgb_lca_comparison.rds")

saveRDS(cm_youden, "confusion_matrix_xgb_lca_youden.rds")
saveRDS(cm_050, "confusion_matrix_xgb_lca_050.rds")
saveRDS(roc_test, "roc_xgb_lca.rds")
saveRDS(auc_ci_boot, "auc_ci_xgb_lca.rds")
saveRDS(varimp_xgb_lca, "varimp_xgb_lca.rds")
