...
View(model_performance_glm_chronic_comparison)

# ============================================================
# 05a_logistic_regression_chronic.R
# Logistic regression model: individual chronic conditions
# ============================================================
...
index <- createDataPartition(
  glm_data_chronic$eurodcat_w6,
  p = 0.8,
  list = FALSE
)

train_data <- glm_data_chronic[index, ]
test_data  <- glm_data_chronic[-index, ]
...
model_performance_glm_chronic <- extract_performance(
  cm = cm_youden,
  model_name = "Logistic regression - individual chronic conditions",
  threshold_label = "Youden",
  threshold_value = youden_threshold,
  auc_value = auc_value
)

model_performance_glm_chronic_050 <- extract_performance(
  cm = cm_050,
  model_name = "Logistic regression - individual chronic conditions",
...
model_performance_glm_chronic_comparison <- bind_rows(
  model_performance_glm_chronic_050,
  model_performance_glm_chronic
)

print(model_performance_glm_chronic_comparison)

# Coefficients / adjusted odds ratios -------------------------------------

glm_chronic_results <- broom::tidy(
  glm_cv_chronic$finalModel,
  exponentiate = TRUE,
  conf.int = TRUE
) %>%
  mutate(across(where(is.numeric), ~ round(.x, 3)))

print(glm_chronic_results, n = Inf)

# Export coefficient table to Word ----------------------------------------

ft_chronic <- flextable(glm_chronic_results) %>%
  autofit()

save_as_docx(
  "Logistic regression - individual chronic conditions" = ft_chronic,
  path = "glm_chronic_results.docx"
)

# Save outputs ------------------------------------------------------------

saveRDS(glm_cv_chronic, "glm_cv_chronic.rds")

saveRDS(youden_coords_train, "youden_coords_train_glm_chronic.rds")
saveRDS(youden_threshold, "youden_threshold_glm_chronic.rds")

saveRDS(model_performance_glm_chronic, "model_performance_glm_chronic.rds")
saveRDS(model_performance_glm_chronic_050, "model_performance_glm_chronic_050.rds")
saveRDS(model_performance_glm_chronic_comparison, "model_performance_glm_chronic_comparison.rds")

saveRDS(cm_youden, "confusion_matrix_glm_chronic_youden.rds")
saveRDS(cm_050, "confusion_matrix_glm_chronic_050.rds")

saveRDS(roc_test, "roc_glm_chronic.rds")
saveRDS(auc_ci_boot, "auc_ci_boot_glm_chronic.rds")

saveRDS(glm_chronic_results, "glm_chronic_results.rds")