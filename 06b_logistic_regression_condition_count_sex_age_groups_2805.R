...
# ============================================================
# 06b_Logistic regression: sex and age groups
...
print(table(glm_data_count$gender, useNA = "ifany"))
...
female_data <- glm_data_count %>%
  filter(gender == 2 | gender == "female" | gender == "Female")

male_data <- glm_data_count %>%
  filter(gender == 1 | gender == "male" | gender == "Male")

age_50_64_data <- glm_data_count %>%
  filter(age2013 >= 50, age2013 <= 64)

age_65_74_data <- glm_data_count %>%
  filter(age2013 >= 65, age2013 <= 74)

age_75plus_data <- glm_data_count %>%
  filter(age2013 >= 75)

# Run subgroup models -----------------------------------------------------

glm_count_female <- run_glm_subgroup(
  data = female_data,
  subgroup_name = "LR Count - Female",
  remove_gender = TRUE
)

glm_count_male <- run_glm_subgroup(
  data = male_data,
  subgroup_name = "LR Count - Male",
  remove_gender = TRUE
)

glm_count_age_50_64 <- run_glm_subgroup(
  data = age_50_64_data,
  subgroup_name = "LR Count - Age 50-64",
  remove_gender = FALSE
)

glm_count_age_65_74 <- run_glm_subgroup(
  data = age_65_74_data,
  subgroup_name = "LR Count - Age 65-74",
  remove_gender = FALSE
)

glm_count_age_75plus <- run_glm_subgroup(
  data = age_75plus_data,
  subgroup_name = "LR Count - Age 75+",
  remove_gender = FALSE
)

# Combine performance results --------------------------------------------

glm_count_subgroup_performance <- bind_rows(
  glm_count_female$performance,
  glm_count_male$performance,
  glm_count_age_50_64$performance,
  glm_count_age_65_74$performance,
  glm_count_age_75plus$performance
)

print(glm_count_subgroup_performance)

glm_count_subgroup_performance_youden <- glm_count_subgroup_performance %>%
  filter(threshold_type == "Youden")

print(glm_count_subgroup_performance_youden)

glm_count_subgroup_performance_050 <- glm_count_subgroup_performance %>%
  filter(threshold_type == "Default 0.50")

print(glm_count_subgroup_performance_050)

# Combine coefficients ----------------------------------------------------

glm_count_subgroup_coefficients <- bind_rows(
  glm_count_female$coefficients,
  glm_count_male$coefficients,
  glm_count_age_50_64$coefficients,
  glm_count_age_65_74$coefficients,
  glm_count_age_75plus$coefficients
)

print(glm_count_subgroup_coefficients, n = Inf)

# Save results ------------------------------------------------------------

saveRDS(
  glm_count_subgroup_performance,
  "glm_count_subgroup_performance_all_thresholds.rds"
)

saveRDS(
  glm_count_subgroup_performance_youden,
  "glm_count_subgroup_performance_youden.rds"
)

saveRDS(
  glm_count_subgroup_performance_050,
  "glm_count_subgroup_performance_default_050.rds"
)

saveRDS(
  glm_count_subgroup_coefficients,
  "glm_count_subgroup_coefficients.rds"
)

write.csv(
  glm_count_subgroup_performance,
  "glm_count_subgroup_performance_all_thresholds.csv",
  row.names = FALSE
)

write.csv(
  glm_count_subgroup_performance_youden,
  "glm_count_subgroup_performance_youden.csv",
  row.names = FALSE
)

write.csv(
  glm_count_subgroup_coefficients,
  "glm_count_subgroup_coefficients.csv",
  row.names = FALSE
)